...
#       #        self.d_k     = config.d_model // config.n_heads   # dim per head
#       #
#       #   4. Create the Q, K, V projection.
#       #      Two approaches (pick one):
#       #
#       #      (a) Single fused projection (more efficient, GPT-2 style):
#       #          self.c_attn = nn.Linear(d_model, 3 * d_model, bias=config.bias)
#       #          Then in forward() you split the output into Q, K, V.
#       #
#       #      (b) Three separate projections (easier to understand):
...
#       #
#       #   5. Output projection:
#       #        self.c_proj = nn.Linear(d_model, d_model, bias=config.bias)
#       #
#       #   6. Dropout layers:
...
#       #
#       #    ⚠️  The transpose(1,2) swaps the T and n_heads dimensions so
